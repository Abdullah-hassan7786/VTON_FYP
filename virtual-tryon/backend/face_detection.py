...
app = FastAPI(title="Rizz-Up Skin Analysis Service", version="2.0.0")
...
# Pre-computed HSV skin tone ranges for performance
...
def classify_skin(b, g, r) -> dict:
    ...
    Classify the raw BGR skin tone into:
      ...
      - chroma     : "clear" | "soft"
    """
    brightness = (0.114 * b + 0.587 * g + 0.299 * r)   # luminance
    warmth     = r - b                                    # warm = high red - blue

    lightness = "light"  if brightness > 170 else ("dark" if brightness < 100 else "medium")
    undertone  = "warm"  if warmth > 20 else ("cool" if warmth < -10 else "neutral")
    # Chroma heuristic: high variance in skin cluster → clear; low → soft
    chroma = "clear"  # will refine below

    return {"lightness": lightness, "undertone": undertone, "chroma": chroma, "brightness": brightness, "warmth": warmth}


def map_to_season(info: dict) -> str:
    """
    Decision tree mapping skin analysis to one of 12 seasons.
    """
    lt  = info["lightness"]
    ut  = info["undertone"]
    br  = info["brightness"]
    ...
    # ── Springs (warm undertone) ──────────────────────────────────────────────
    if ut == "warm":
        if lt == "light":
            return "Light Spring" if wm < 40 else "Warm Spring"
        elif lt == "medium":
            return "Bright Spring" if br > 130 else "Warm Autumn"
        else:
            return "Deep Autumn"

    # ── Winters (cool undertone) ──────────────────────────────────────────────
    elif ut == "cool":
        if lt == "light":
            return "Light Summer" if br > 160 else "Cool Summer"
        elif lt == "medium":
            return "Cool Winter" if br > 115 else "Bright Winter"
        else:
            return "Deep Winter"

    # ── Neutral → split between Summer and Autumn ────────────────────────────
    else:
        if lt == "light":
            return "Soft Summer" if br > 165 else "Light Summer"
        elif lt == "medium":
...
def crop_to_face(img: np.ndarray) -> np.ndarray:
    ...
    if face is not None:
        ...
        # Centre-square crop biased ¼ from top to favour face region
        side = min(w, h)
        x1   = (w - side) // 2
        y1   = (h - side) // 4
        x2   = x1 + side
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "rizz-up-face-detection"}
...
if __name__ == "__main__":
    print("=" * 60)
    print("  Rizz-Up Face Detection & Skin Analysis Service")
